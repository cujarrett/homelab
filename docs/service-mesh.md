...
> This section is for AI agents. A human can skip ahead.

**Hardware:** 4× Raspberry Pi 5, ARM64, NVMe SSD, k3s v1.x. Nodes: `ctrl-1`
(192.168.10.100, server), `work-1/2/3` (192.168.10.101–103, agents).

**CNI:** k3s default (Flannel + klipper-lb). Not Cilium. Not Calico. This matters — see
[One-Way Doors](#one-way-doors).

**Ingress:** Traefik deployed as a DaemonSet via `HelmChartConfig`, binding hostPorts 80
and 443 on every node. TLS via cert-manager. External traffic enters via Cloudflare Tunnel
(`cloudflared`, 2 replicas in `cloudflare` namespace) pointing at `https://192.168.10.101:443`.

**Observability:** kube-prometheus-stack (Prometheus + Grafana + Alertmanager) in
`monitoring`. Loki + Promtail for logs. Grafana dashboards loaded via ConfigMap sidecar
with label `grafana_dashboard: "1"`.

**Messaging:** NATS JetStream (3-replica cluster) in `nats` namespace. NACK controller
manages Stream and Consumer CRDs. Apps connect on port 4222 (TCP, not HTTP).

**Storage:** Longhorn. Three StorageClasses: `longhorn` (Delete, default),
`longhorn-retain` (Retain), `longhorn-delete` (explicit Delete).

**Service-to-service today:** Plain Kubernetes Services. No mTLS. No traffic policy. No
per-route metrics. Prometheus scrapes apps directly via ServiceMonitor.
...
The Crossplane platform lives in `platform/` and exposes these composite types:

| Kind | What it creates |
|---|---|
| `XApi` | Deployment + Service + ServiceAccount + optional Ingress + optional XCache + ServiceMonitor |
| `XSpa` | nginx Deployment + Service + ConfigMap (nginx.conf generated by composition) + optional Ingress |
| `XWordpress` | MariaDB StatefulSet + WordPress Deployment + PVCs + Services |
| `XTopic` | NATS JetStream Stream via NACK |
| `XSubscription` | NATS durable Consumer cursor via NACK |
| `XCache` | Redis StatefulSet (in-cluster) or ElastiCache reference (cloud) |
| `XSql` | Postgres StatefulSet (in-cluster) or RDS reference (cloud) |
| `XObjectStorage` | S3 bucket + scoped IAM credentials |
| `XNoSql` | DynamoDB table + scoped IAM credentials |

All compositions use `function-go-templating` to render resources inline. Credentials reach
pods as mounted files at `/bindings/<name>/` (servicebinding.io convention), not env vars.

`XApi` uses init containers to block pod startup until each binding Secret exists. Namespaces
are pre-created by tenant `namespace.yaml` files — not by the compositions themselves.

Active instances:

| Namespace | XR kind | Host |
|---|---|---|
| `mattjarrett-com` | `XWordpress` | mattjarrett.com (public) |
| `mattjarrett-dev` | `XSpa` | mattjarrett.dev (public) |
| `blog` | raw Deployment | blog.mattjarrett.dev (public) |
| `my-vinyl` | `XSpa` + `XApi` + `XCache` | myvinyl.mattjarrett.dev (public) |
| `js-pollock` | `XSpa` | jspollock.mattjarrett.dev (public) |
| `sump-pump` | `XApi` ×2 + `XTopic` + `XSubscription` | internal only |
