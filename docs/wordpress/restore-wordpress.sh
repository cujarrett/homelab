...
# mu-plugins is a read-only ConfigMap mount the composition provides, not user data.
# A backup taken from a live pod captures that file, but the restore target mounts it
# read-only too, so tar can't overwrite it - excluding it is correct, not a workaround.
