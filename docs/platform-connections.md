...
- **Enrollment vs. enforcement are two independent dials.** Being *in* the mesh (sidecar injected) is separate from being *locked down* (`STRICT` + deny + `REGISTRY_ONLY`). Rollout enrolls a namespace in permissive mode first (nothing denied, full telemetry) to inventory its real connections, then flips to enforced once every observed flow is registered.
  - **Trap (learned the hard way): a bare ALLOW `AuthorizationPolicy` is itself an enforcement switch.** Istio treats any ALLOW policy selecting a workload as deny-by-default for that workload — everything not matching the ALLOW is denied. So a `ConnectionGrant`/ALLOW rendered for a workload that is *enrolled but not yet meant to be enforced* silently starts denying traffic the moment that workload gets a sidecar (e.g. on a pod recreation). Under permissive mTLS the ALLOW's `principals` requirement can't be met by plaintext callers either, compounding it. **Never render a Grant/ALLOW into a namespace until that namespace is deliberately being enforced** (STRICT mTLS + baseline allows in place). Enrollment must stay policy-free; the Grant is the enforcement act, not a passive annotation.
